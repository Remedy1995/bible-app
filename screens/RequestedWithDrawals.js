...
const RequestedWithdrawals = () => {
    ...
    return (
        <><PageGradient>
            <FlatListHeader subHeader="Withdrawal Requests" subHeaderActive="See All" style={{ borderRadius: 15, left: width > 360 ? 7 : 0, width: width > 360 ? 360 : 305 }} />
            <Input placeholder="Search by Typing Account Number" label="" style={[formData.hasError
                ...
                width: width > 360 ? 350 : 300, top: -20, right: 13
            }]} textConfigs={{
            ...
            {showLoader ? <PageLoader PageStyle={{
                marginLeft: 155,
                marginTop: 50
            }} /> : formData.accountnumber !== '' && searchActivity?.length === 0 ? (<Text style={{ textAlign: 'center' }}> No search results found</Text>) : (<FlatList data={searchActivity.filter((data)=> !data.transactiondetails.split(' ').includes('deposited'))} renderItem={(item) => ActivityRenderFlatList(item)} keyExtractor={(item) => item._id} />)}
            {/* </MiddleCard> */}
            {/* </Card>  */}
